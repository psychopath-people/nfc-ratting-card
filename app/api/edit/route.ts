...
export async function POST(req: NextRequest) {
  try {
    const { code, currentPin, cafeName, mapsUrl, reviewUrl, newPin, waNumber, reviewMode } = await req.json()
    ...
    const mode = reviewMode === 'direct' ? 'direct' : reviewMode === 'filtered' ? 'filtered' : undefined
    await editCard(code, cafeName.trim(), finalUrl, newPinHash, waNumber?.trim(), mode)
